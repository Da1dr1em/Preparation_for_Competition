  ...
  always @( posedge clk or negedge rst_n ) begin
    if ( !rst_n ) begin
      ...
      w[0] <= #1 kld ? key[000:031] : 
                 ( enable ? w[0]^subword^{rcon,24'h000000} : w[0] );
    end
  end
  always @( posedge clk or negedge rst_n ) begin
    if ( !rst_n ) begin
      ...
      w[1] <= #1 kld ? key[032:063] : 
                 ( enable ? w[0]^w[1]^subword^{rcon,24'h000000} : w[1] );
    end
  end
  always @( posedge clk or negedge rst_n ) begin
    if ( !rst_n ) begin
      ...
      w[2] <= #1 kld ? key[064:095] : 
                 ( enable ? w[0]^w[2]^w[1]^subword^{rcon,24'h000000} : w[2] );
    end
  end
  always @( posedge clk or negedge rst_n ) begin
    if ( !rst_n ) begin
      ...
      w[3] <= #1 kld ? key[096:127] : 
                 ( enable ? w[0]^w[3]^w[2]^w[1]^subword^{rcon,24'h000000} : w[3] );
    end
  end
